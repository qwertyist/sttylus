    ...
    <b-overlay :show="loading">
      <b-jumbotron header="Välj användare">
        <template #lead v-if="desktop">
          ...
          (<code>sttylus.se/app/</code>) dröjer tills den är vattentät i alla
          riktningar. Tills dess loggar du in som "Skrivtolk" och du kan
          fortfarande importera dina förkortningslistor från webappen.<br />
        </template>
        <div v-if="desktop && users">
          <b-list-group v-for="user in users" v-bind:key="user['id']"
            ><b-list-group-item href="#" @dblclick="loginAs(user)"
              ><b-button variant="primary" @dblclick="loginAs(user)"
                >Logga in</b-button
              >
        ...
        <div v-if="!desktop">
          ...
          <div v-if="step == 1">
            <div v-if="passwordprompt == 'sync'">
              STTylus kommer att synka dina lokala filer med användardata som
              finns på webben.
            </div>
            ...
            <div
              v-if="passwordprompt == 'register' || passwordprompt == 'reset'"
            >
              <ValidationObserver v-slot="{ handleSubmit }">
                <b-form @submit.prevent="handleSubmit(onSubmit)">
                  <ValidationProvider
  ...
  methods: {
    ...
    onLogin() {
      this.$store
        ...
        .catch((err) => {
          this.$toast.warning("Du har angett ett felaktigt lösenord")
