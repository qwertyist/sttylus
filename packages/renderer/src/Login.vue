<template>
    <div>
        ...
        <b-overlay :show="loading">
            <b-jumbotron header="Välj användare">
                <template #lead v-if="desktop">
                    Möjligheten att logga in med användarkontot från webappen
                    (<code>sttylus.se/app/</code>) dröjer tills den är vattentät
                    i alla riktningar. Tills dess loggar du in som "Skrivtolk"
                    och du kan fortfarande importera dina förkortningslistor
                    från webappen.<br />
                </template>
                <div v-if="desktop && users">
                    <b-list-group v-for="user in users" v-bind:key="user['id']"
                        ><b-list-group-item href="#" @dblclick="loginAs(user)"
                            ><b-button
                                variant="primary"
                                @dblclick="loginAs(user)"
                                >Logga in</b-button
                            >
                ...
                <div v-if="!desktop">
                    ...
                    <div v-if="step == 1">
                        <div v-if="passwordprompt == 'sync'">
                            STTylus kommer att synka dina lokala filer med
                            användardata som finns på webben.
                        </div>
                        <div
                            v-if="
                                passwordprompt == 'login' ||
                                passwordprompt == 'sync'
                            "
                        >
                        ...
                        <div v-if="passwordprompt == 'register'">
                            I och med att detta är din första inloggning så
                            väljer du ditt lösenord nedan.
                        </div>
                        <div v-if="passwordprompt == 'reset'">
                            Lösenordet för ditt konto har nollställts. Skapa ett
                            nytt lösenord nedan.
                        </div>
                        <div
                            v-if="
                                passwordprompt == 'register' ||
...
export default {
    ...
    methods: {
        next() {
            api.isRegistered(this.form.email)
                .then((resp) => {
                    console.log(
                ...
                .catch((err) => {
                    if (err.response) {
                        if (err.response.status == 502) {
                            if (this.users) {
                                this.$bvModal.msgBoxOk(
                                    'Du är inte ansluten till internet. Logga in på en befintlig lokal användare.',
                                    {
                            ...
                            this.$bvModal.msgBoxOk(
                                'Du är inte ansluten till internet och har heller ingen användare lokalt. Kontakta supporten för att få hjälp.',
                                {
                        ...
                        this.error =
                            'Inget användarkonto med den e-postadressen'
                    }
                })
        },
        onSubmit() {
            ...
            if (this.passwordprompt == 'register') {
                api.register(data).then(() => {
                    ...
                    this.$store
                        ...
                        .then(() => {
                            this.loading = true
                            api.createStandard(this.$store.state.userData.id)
                                .then((resp) => {
                                    this.$store.commit(
                                        'setSelectedStandard',
                                        resp.data
                ...
                api.resetPassword(this.form.email, this.form.password).then(
                    (resp) => {
                        console.log(resp.data)
                        this.$store
                            .dispatch('AUTH_REQUEST', {
        ...
        onSync() {
            ...
            this.$store
                ...
                .catch((err) => {
                    ...
                    this.validPassword = false
                    this.$toast.error(
                        'Kunde inte synkronisera listor. Kontakta admin',
                        err
                    )
                })
        },
