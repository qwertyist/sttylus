...
function renderPreview(index, q, action = 'paragraph') {
    ...
    switch (action) {
        ...
        case 'word':
            let word =
                manuscript.text[manuscript.line].split(' ')[manuscript.word]
    ...
    switch (action) {
        ...
        case 'insertWord':
            ...
            if (
                ...
                console.log(
                    'Add one to offset, because new line, first word, first op'
                )
                offset += 1
                newLine = true
            }
            ...
            if (manuscript.word + 1 < insert.split(' ').length) {
                ...
                if (!skip) {
                    q.insertText(
                        (newLine ? 1 : 0) + index + chunk.length,
                        ' ',
                        attr
                    )
                }
                ...
                if (manuscript.op < curr_line.ops.length - 1) {
                    ...
                    manuscript.word = 0
                    return (
                        offset + promptManuscript(index + offset - 1, q, action)
                    )
                } else {
            ...
            if (manuscript.word + 1 > curr_op.insert.split(' ').length) {
                if (manuscript.lookahead) {
                    ...
                    if (matches == null) {
                        ...
                        return (
                            offset +
                            promptManuscript(index + offset - 1, q, action)
                        )
                    } else {
