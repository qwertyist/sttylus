  ...
  <div>
    ...
    >
      <form @submit.prevent="onSubmit" inline autocomplete="off">
        <b-form-input required v-model="form.title" autofocud  placeholder="Titel..." type="text" />
        <b-form-input v-model="form.id" hidden />
        <div class="float-right">
          <b-button variant="danger" type="reset" @click="hide">Avbryt</b-button>
  ...
  methods: {
    ...
    onSubmit(evt) {
      ...
      if (!this.createNew && this.form.id != "new") {
        api.updateManuscript(this.form).then(resp => {
          EventBus.$emit("savedManuscript", { id: resp.data.id, close: this.close });
        }).catch(err => {
          console.log("failed updating manuscript", err)
        })
      } else {
        api.createManuscript(this.form).then(resp => {
          EventBus.$emit("savedManuscript", { id: resp.data.id, close: this.close });

        }).catch(err => console.log("create manuscript failed:", err))
      }
