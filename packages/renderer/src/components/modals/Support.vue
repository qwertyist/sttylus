                    ...
                    <b-row>
                        ...
                        <b-col cols="2">
                            <b-button
                                size="sm"
                                v-if="sharingList"
                                @click.prevent="onLeaveSharedList"
                                >Lämna</b-button
                            >
                        </b-col>
                        <b-col>
        ...
        <div v-if="sharedAbbs">
            ...
            <div v-if="show.sharedAbbs">
                <span v-for="letter in letters" :key="letter" class="p-2 mb-3">
                    ...
                    <a v-else href="#" @click="setCurrentLetter(letter)">{{
                        letter
                    }}</a>
                </span>
...
export default {
    ...
    methods: {
        ...
        filterMissedAbbs() {
            ...
            missedAbbs.forEach((a) => {
                console.log(a)
                this.filteredMissedAbbs.push(
                    this.$store.state.missedAbbs.get(a)
                )
            })
        },
        closeModal() {
        ...
        onSubmitSharedList() {
            if (this.form.sharedList != '' && this.selectedBaseList == null) {
                api.joinSharedList(this.form.sharedList.trim())
                    .then((resp) => {
                    ...
                    .catch((err) => {
                        this.$toast.error(
                            'Kunde inte gå med i delad lista:',
                            err
                        )
                        this.form.sharedList = ''
                ...
                api.initSharedList(this.selectedBaseList)
                    .then((resp) => {
        ...
        onSubmitSharedAbb() {
            ...
            if (abb != '') {
                if (abb == word) {
                    api.deleteSharedAbb(this.form.sharedList, abb)
                        .then((resp) => {
                            this.form.sharedAbb = ''
                            ...
                            if (obj.base) {
                                api.deleteAbb(obj.base, obj)
                                    .then((resp) => {
                                        console.log(
                                            'shared abb also deleted user abb:',
                                            resp.data
                                        )
                                    })
                                    .catch((err) => {
                                        console.error(
                                            'shared abb couldnt delete user abb',
                                            err
                                        )
                                    })
                            }
                        ...
                        .catch((err) => {
                            this.$toast.error(
                                'Ta bort: Den delade listan existerar inte'
                            )
                    ...
                    api.createSharedAbb(this.form.sharedList, obj)
                        .then(() => {
                            this.form.sharedAbb = ''
                            this.form.sharedWord = ''
                            this.getSharedAbbs()
                            ...
                            if (obj.base) {
                                ...
                                api.createAbb(obj.base, obj)
                                    .then((resp) => {
                                        console.log(
                                            'shared abb also created user abb:',
                                            resp.data
                                        )
                                        api.cacheAbbs().then(() => {})
                                    })
                                    .catch((err) => {
                                        console.error(
                                            'shared abb couldnt create user abb',
                                            err
                                        )
                                    })
                            }
                        ...
                        .catch((err) => {
                            this.$toast.error(
                                'Den delade listan existerar inte'
                            )
                            this.form.sharedList = ''
                        })
                }
            ...
            if (this.lookupPhrase !== '') {
                api.lookup(this.lookupPhrase)
                    .then((resp) => {
                        ...
                        var results = []
                        var i = 0
                        for (const [list, matches] of Object.entries(
                            resp.data
                        )) {
                            if (matches.length == 0) {
                                this.lookupResults = [
                            ...
                            if (matches.length > 50) {
                                this.lookupResults = [
                                    { id: -500, match: 'För många träffar' },
                                ]
                                return
                            }
    ...
    mounted() {
        ...
        if (sharedListId != '') {
            api.joinSharedList(sharedListId)
                .then((resp) => {
                    this.form.sharedList = resp.data
                    this.$nextTick(() => {
                        this.sharingList = true
