<template>
  ...
  >
    <b-row v-if="collab">
      <b-col cols="6">
        <h4>Lägg till delad förkortning</h4>
      </b-col>
      <b-col>
        <b-form @submit.prevent="onSubmitSharedList" autocomplete="off">
          <b-row>
            <b-col cols="2"></b-col>
            <b-col cols="2">
              <b-button size="sm" v-if="sharingList" @click.prevent="onLeaveSharedList">Lämna</b-button>
            </b-col>
            <b-col>
              <b-form-select size="sm" :disabled="form.sharedList != ''" v-model="selectedBaseList">
                <option value="null">Ny lista</option>
                <option
                  v-for="list in selectedLists"
                  :value="list.id"
                  :key="list.id"
                  :disabled="form.sharedList != ''"
                >{{ list.name }}</option>
              </b-form-select>
              <b-form-input
                :disabled="sharingList"
                v-model="form.sharedList"
                id="list"
                size="sm"
                placeholder="Skapa/gå med i delad lista"
                ref="sharedListInput"
                spellcheck="false"
              />
            </b-col>
    ...
    <div v-if="collab">
      <b-form
        @submit.prevent="onSubmitSharedAbb"
        autocomplete="off"
        class="d-flex justify-content-between align-items-center"
      >
        <b-form-input
          v-model="form.sharedAbb"
          :disabled="!sharingList"
          id="abb"
        ...
        />
        <b-form-group>
          <b-badge class="mt-3">Fokusera med CTRL+1</b-badge>
        </b-form-group>
    ...
    <div v-if="sharedAbbs">
      <div class="d-flex justify-content-between align-items-center">
        <span
          style="font-size: 1.5rem; margin-bottom:0.5rem; font-weight: 500; line-height: 1.2;"
        >Delade förkortningar</span>
        <span>
          <b-badge class="float-right">Visa/dölj med CTRL+2</b-badge>
          <div v-if="show.sharedAbbs">
            <b-badge>Växla bokstav med CTRL+Bokstav/Noll</b-badge>
          </div>
          <br />
        </span>
        <b-col v-if="!collab && selectedBaseList">
          <small class="mt-3">
            Du är nedkopplad, men kan fortfarande spara delade
            förkortningar.
          </small>
          <b-button
            class="float-right"
            ...
            v-if="sharingList"
            variant="primary"
            @click.prevent="onSaveSharedAbbs"
          >Spara</b-button>
        </b-col>
      </div>
      <div v-if="show.sharedAbbs">
        <span v-for="letter in letters" :key="letter" class="p-2 mb-3">
          <b v-if="letter == currentLetter">{{ letter }}</b>
          <a v-else href="#" @click="setCurrentLetter(letter)">{{ letter }}</a>
        </span>
        <b-list-group ref="sharedAbbsGroup">
          <b-list-group-item
            class="p-1 pl-3"
          ...
          >
            ...
            <b-button
              size="sm"
              variant="danger"
              @click.prevent="onRemoveSharedAbb(abb.abb)"
              class="float-right"
            >
              <b-icon icon="trash" />
            </b-button>
    ...
    <div v-if="show.missedAbbs">
      <b-list-group style="overflow-y: auto; overflow-x: hidden;  max-height: 40vh;">
        <b-list-group-item
          class="p-0 pl-2"
        ...
        >
          <b-row>
            ...
            <b-col>
            <b-button size="sm" @click="dontRemindMissed(abb.abb)" variant="warning">Påminn inte</b-button>
            </b-col>
          </b-row>
        </b-list-group-item>
      </b-list-group>
      ...
      <b-list-group size="sm" style="max-height: 40vh; overflow-y: auto">
        ...
        >
          ...
          <span class="float-right">
            <b-button size="sm" variant="primary" @click="addAbb(s)">Lägg till</b-button>
            <b-button size="sm" @click="ignoreSuggestion(s)">Ignorera</b-button>
          </span>
        </b-list-group-item>
      </b-list-group>
      <div class="float-right">
        <b-button variant="danger" size="sm" @click="ignoreAllSuggestions">Ignorera alla</b-button>
      </div>
    </div>
    <div v-else>
      <b-badge>...</b-badge>
...
export default {
  ...
  methods: {
      ...
      if (this.form.sharedList != "" && this.selectedBaseList == null) {
          ...
          .then((resp) => {
            ...
            if (resp.data != this.form.sharedList) {
              ...
              if (this.selectedBaseList) {
                this.$toast.warning("Bara skaparen av en delad lista kan dela en befintlig lista")
        ...
        api
          ...
          .then((resp) => {
            ...
            this.$store.commit("setSharedList", { id: resp.data, base: this.selectedBaseList });
      ...
      if (abb != "") {
        if (abb == word) {
          api.deleteSharedAbb(this.form.sharedList, abb).then((resp) => {
            ...
            if (obj.base) {
              api.deleteAbb(obj.base, obj).then(resp => {
          ...
          obj = { ...obj, create: true, abb: abb, word: word, me: true };
          if (word == "") {
            obj.word = abb
            obj.create = false
            obj.override = true
          ...
          api.createSharedAbb(this.form.sharedList, obj).then(() => {
            this.form.sharedAbb = "";
            this.form.sharedWord = "";
            this.getSharedAbbs();
    ...
    onRemoveSharedAbb(abb) {
      api.deleteSharedAbb(this.form.sharedList, abb).then((resp) => {
        this.getSharedAbbs();
        let obj = { base: this.selectedBaseList, delete: true, abb: abb, me: true }
        EventBus.$emit("sharedAbb", obj);
        if (obj.base) {
          api.deleteAbb(obj.base, obj).then(resp => {
            console.log("shared abb also deleted user abb:", resp.data)
