    ...
    <b-tabs v-model="tabIndex" content-class="subTab">
      <b-tab title="Välj/visa" class="mt2">
        <b-row>
          <b-col cols="3">
            ...
            <div>
              <b-form autocomplete="off">
                <b-list-group style="margin-bottom: 10px; max-height: calc(100vh - (42px*4); overflow-y: auto">
                  <b-list-group-item v-for="(s, index) in selectedManuscripts" v-bind:key="index">
                    <b-row>
                      ...
                      <b-col class="align-self-center">
                        <b-input-group>
                          <b-input
                            ...
                            v-model="s.abb"
                            :formatter="abbFormatter"
                            @focus="setCurrentAbb(index, s.abb)"
                            @blur="setManuscriptAbb(index, s.abb)"
                          ></b-input>
                          <b-input-group-append>
                            <b-button
                              style="margin: 0px"
                              variant="danger"
                              @click="deselectManuscript(index, '')"
                            >x</b-button>
                          </b-input-group-append>
                        </b-input-group>
                      </b-col>
          ...
          <b-col cols="9">
            <div>
              <b-list-group scrollable style="max-height: calc(100vh - (50px*3)); overflow-y: auto">
                <b-list-group-item v-for="m in manuscripts" v-bind:key="m.id">
                  <b-row>
                    <b-col>
                      <h4>{{ m.title }}</h4>
                    </b-col>
                    <b-col>
                      <b-form-group class="float-right">
                        ...
                        <b-button
                          class="float-right"
                          variant="danger"
                          @click="openConfirmRemoveModal(m)"
                        >Ta bort</b-button>
                      </b-form-group>
                    </b-col>
                  </b-row>
        ...
        >
          ...
          <div class="preview" style="height: 45vh !important">
            <span class="preview" v-html="selectedForRemoval.preview"></span>
          </div>
          <hr />
          <b-button
            class="float-right"
          ...
          <b-button class="float-right" variant="danger" @click="removeManuscript()">Ta bort</b-button>
        </b-modal>
        <b-modal
        ...
        >
          <p>Vill du spara manuset du redigerar innan du skapar ett nytt?</p>
        ...
        >
          ...
          <b-button class="float-right" variant="primary" @click="saveAndClose()">Spara</b-button>
          <b-button class="float-right" variant="danger" @click="dontSaveAndClose()">Stäng utan att spara</b-button>
        </b-modal>
      </b-tab>
      <b-tab
      ...
      <template v-if="id && id !== 'parsed' && id !== 'new'" v-slot:tabs-end>
        ...
        <b-nav-item href="#" role="presentation" @click="openConfirmCreateNewModal()">Skapa nytt</b-nav-item>
      </template>
    </b-tabs>
  </div>
</template>
  ...
  methods: {
    ...
    deselectManuscript(m) {
      ...
      if (this.selectedManuscripts.length > 0) {
        ...
        if (/text\d+/.test(abb)) {
          console.log("auto generated abb [", abb, "], proceed with caution");
    ...
    abbFormatter(value) {
      ...
      if (index !== -1) {
        this.selectedManuscripts[this.currentAbb[0]].abb = this.currentAbb[1];
      } else {
    ...
    updateStore() {
      console.log("updateStore, view selectedManuscripts", this.selectedManuscripts)
    ...
    removeManuscript() {
      api.deleteManuscript(this.selectedForRemoval.id).then((response) => {
        let index = this.selectedManuscripts.findIndex(
          (manuscript) => manuscript.id === this.selectedForRemoval.id
        );
        if (index > -1) {
