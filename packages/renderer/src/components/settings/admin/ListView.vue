<template>
  <div>
    ...
    <div v-if="show.toolbox">
      <b-button @click="$bvModal.show('batchImportModal')">Importera till användare</b-button>
    </div>
    <div v-else>
      <b-badge>...</b-badge>
    </div>
    ...
    <div v-if="batchImport.upload">
      ...
      <div v-else>
        <b-form @submit.prevent="confirmBatchImport" @reset.prevent="resetBatchImport">
            <b-list-group v-for="pending in pendingLists" :key="pending.creator + '_' + pending.id">
              <b-list-group-item>
                <div class="d-flex justify-content-between align-items-center">
                  <span>{{pending.creator_name}}</span><span>{{pending.name}}</span>
                  <span v-if="!pending.clean">
                    <b-badge variant="success">{{pending.new}}</b-badge>
                    <b-badge variant="danger">{{pending.conflicts.length}}</b-badge> 
                    <b-button variant="warning" @click="confirmSingleImport(pending)">Importera</b-button>
                  </span>
                  <span v-else>
                    <b-badge variant="success">{{pending.new}}</b-badge>
                    <b-button @click="confirmSingleImport(pending)">Skapa</b-button>
                  </span>
                </div>
              </b-list-group-item>
            </b-list-group>
...
export default {
  ...
  methods: {
    ...
    uploadBatchImport() {
      ...
      this.batchImport.files.forEach(file => {
          ...
          fr.onload = (e) => {
            ...
            this.batchImport.creators.set(pending.meta.creator, this.users.find(u => u.id == pending.meta.creator))
            if (pending.abbs) {
              if (pending.abbs.length > 0) {
                api.getList(pending.meta.id)
                .then(resp => {
                  if(resp.status != 200) { 
                    const pendingListInfo = {
                      creator: pending.meta.creator,
                      creator_name: this.getCreator(pending.meta.creator, pending.filename),
    ...
    detectConflicts(curr, pending) {
      ...
      api.getAbbs(curr.id)
        .then(resp => {
          resp.data.map(abb => { currAbbs.set(abb.abb, abb) })
          ...
          pending.abbs.map(newAbb => { 
            ...
            if(currAbb) {
              if (currAbb.word != newAbb.word) {
                conflicts.push({ abb: newAbb.abb, old: currAbb.word, new: newAbb.word})
              }
            }
          })

          if(pending.abbs.length != resp.data.length && conflicts.length > 0) {
            let creator_name = this.getCreator(pending.meta.creator, pending.filename)
            const pendingListInfo = { 
                name: curr.name,
                id: curr.id,
      ...
      .catch(err => {
        console.error("detectConflicts failed after getting current abbs", err)
      })
    ...
    confirmSingleImport(pendingList) {
      ...
      if (pendingList.clean) { 
        ...
        api.createList(list.meta)
        .then(resp => {
          let created = resp.data
          api.importList(created.id, list.abbs)
          .then(resp => {
        ...
        api.importList(list.meta.id, list.abbs)
        .then(resp => {
          let i = this.pendingLists.findIndex(l => l.id == list.meta.id)
          this.pendingLists.splice(i, 1)
          this.batchImport.lists.delete(list.meta.id)
