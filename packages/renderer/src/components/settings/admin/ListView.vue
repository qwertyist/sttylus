        ...
        <div v-if="show.lists">
            ...
            >
                ...
                ><template #cell(download)="row">
                    <b-button
                        @click="downloadList(row.item.id, row.item.owner)"
                    >
        ...
        detectConflicts(curr, pending) {
            ...
            api.getAbbs(curr.id)
                .then((resp) => {
                    ...
                    if (
                        pending.abbs.length != resp.data.length &&
                        conflicts.length > 0
                    ) {
                ...
                api.createList(list.meta)
                    .then((resp) => {
                        ...
                        api.importList(created.id, list.abbs)
                            .then((resp) => {
                                let i = this.pendingLists.findIndex(
                                    (l) => l.id == list.meta.id
                                )
                            ...
                            .catch((err) => {
                                console.error(
                                    'couldnt import to created list:',
                                    err
                                )
                            })
                    })
                    .catch((err) => {
                        console.error(
                            'couldnt create list before importing:',
                            err
                        )
                    })
                return
        ...
        downloadList(id, owner) {
            ...
            api.getList(id).then((resp) => {
                ...
                api.getAbbs(id).then((resp) => {
                    ...
                    link.download =
                        owner + '_' + exportedList.meta.name + '.json'
                    link.click()
                })
            })
        },
