<template>
  <div>
    <b-row>
      <b-col cols="4" style="height: 85vh !important; overflow-y: auto">
        <h3 class="float-left">Dina listor</h3>
        <div v-if="!tester">
          <b-button
            size="sm"
            class="float-right"
        ...
        >
          ...
          <template #cell(checkbox)="data">
            <kbd v-if="selectedAddons.indexOf(data.item.id) != -1">{{
              data.index + 1
            }}</kbd>
            <b-form-checkbox
      ...
      <b-col>
        ...
        -->
              ...
              <template v-if="viewedList.type < 2" #cell(remove)="row">
                <!-- <b-button
    ...
    <AddList />
    <RemoveList :list="viewedList" :abbs="abbs" />
...
export default {
  ...
  computed: {
    ...
    orderedAddonLists() {
      const selected = this.addonLists.filter(
        (list) => this.selectedAddons.indexOf(list.id) !== -1
      )
      const orderedSelected = selected.sort(
        (a, b) =>
          this.selectedAddons.indexOf(b.id) - this.selectedAddons.indexOf(a.id)
      )
      const rest = this.addonLists.filter(
        (list) => this.selectedAddons.indexOf(list.id) === -1
      )
  ...
  methods: {
    ...
    quickSelectStandard(i) {
      ...
      this.$nextTick(() => {
        ...
        this.$toast.info('Byter till ' + this.standardLists[i - 1].name, {
          duration: 750,
        })
      })
    },
      ...
      if (!list) {
        ...
        if (
          ...
          if (this.$store.getters.getModalOpen) {
            this.$toast.warning(
              'Kom ihåg att välja listan om du vill lägga till förkortningar i den.',
              { duration: 2000 }
            )
          }
        }
        ...
        .then((list) => {
          api
            ...
            .then((abbs) => {
              ...
              const action = list.data.type < 3 ? 'Ta bort lista' : 'Dölj lista'
              this.removelist = {
                action: action,
                id: list.data.id,
                name: list.data.name,
