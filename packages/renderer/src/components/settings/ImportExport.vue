                            ...
                            <b-row>
                                ...
                                <b-col>
                                    <span style="font-size: 1.2em"
                                        ><b-badge size="md">{{
                                            standard.value.split('-')[0]
                                        }}</b-badge></span
                                    >
                                    <b-button
                                        class="float-right"
                                        size="sm"
                                        @click="exportSTTylus(standard.value)"
                                        >Exportera</b-button
                                    >
                                </b-col>
                                <b-col>
                            ...
                            <b-row>
                                ...
                                <b-col>
                                    ...
                                    <b-button
                                        class="float-right"
                                        size="sm"
                                        @click="exportSTTylus(addon.value)"
                                        >Exportera</b-button
                                    >
                                </b-col>
                                <b-col>
                    ...
                    >
                        ...
                        <b-form-checkbox-group v-model="exportForm.addons">
                            ...
                            >
                                <b-row>
                                    <b-col cols="9">
                                        <b-form-checkbox :value="addon.value">{{
                                            addon.text
                                        }}</b-form-checkbox>
                                    </b-col>
                                    <b-col>
            ...
            <div v-if="!confirm">
                ...
                <div v-if="from == 'protype'">
                    ...
                    <b-jumbotron>
                        <template #lead>
                            <p>
                                För import av listor och användare från ProType
                                rekommenderas
                                <a href="https://sttylus.se/utx/"
                                    >UTX-programmets Förkortningsverktyg</a
                                >.<br />
                                Där kan du konvertera från flera filtyper till
                                textfilsformat.
                            </p>
                        </template>
                        Annars hittar du din användare under
                        <code>Users/</code> där du har ProType installerat. För
                        varje förkortningslista finns ytterligare en katalog och
                        när du hittat din lista det filen som heter antingen
                        <code>wordlist</code> eller <code>wordlist.dat</code> du
                        ska välja för uppladdning i formuläret nedan.
                    </b-jumbotron>
                    <div v-if="online">
                ...
                <div v-if="from == 'textontop' || from == 'illumitype'">
                    ...
                    <div v-else>
                        ...
                        <b-form @submit.prevent="selectUploadedList">
                            <b-select
                                v-model="selectedJSONList"
                                @change="changeSelectedJSONList"
                            >
                                <b-select-option
                            ...
                            <br />
                            <br />
                            Det finns {{ countJSONAbbs }} förkortningar i denna
                            lista.
                            <br />
...
förkortning=fras"
            ...
            <div v-if="confirm || safe">
                ...
                <b-form autocomplete="off">
                    <div style="height: 68vh !important; overflow-y: auto">
                        <b-table
                            small
                            responsive
                            hover
                            ...
                            <template v-slot:cell(issue)="row">
                                ...
                                <span v-if="row.item.notduplicate != null">
                                    ...
                                    <span
                                        v-else-if="
                                            ...
                                            '__importduplicate__'
                                        "
                                        >Dubblett vid importering</span
                                    >
                                    <span v-else>
                                        <b-form-checkbox
                                            v-model="row.item.overwrite"
                                            @change="
                                                changeOverwriteAbb(row.item)
                                            "
                                        >
                                            Ersätt
                                            <i>{{ row.item.issue }}</i>
                                ...
                                <span
                                    v-if="
                                        ...
                                        (row.item.validword != null ||
                                            row.item.validabb != null)
                                    "
                                >
                                    ...
                                    <span
                                        v-if="
                                            row.item.issue == '__formaterror__'
                                        "
                                        >Fel format vid importering</span
                                    >
                    ...
                    <b-button @click="resolveConflicts(null)"
                        >Kontrollera fel/konflikter</b-button
                    >
                </b-form>
            </div>
        </b-col>
    ...
    methods: {
        ...
        importSTTylus(short_id) {
            ...
            if (this.$mode == 'desktop') {
                axios
                    .get('https://sttylus.se/api2/abbs/public/' + this.form.id)
                    .then((resp) => {
            ...
            api.importSTTylus(this.form.id).then((resp) => {
                this.$toast.info('Hämtar listan...')
                this.confirm = true
                this.populateAbbs(resp.data.abbs)
            })
            ...
            api.getList(list)
                .then((resp) => {
                    api.exportSTTylus(resp.data)
                        .then((resp) => {
                            this.$toast.success(
                                'Listan går att importera online/offline med ID-numret till vänster'
                            )
                        })
                        .catch((err) => {
                            this.$toast.warning(
                                'Listan kunde inte exporteras:' + err,
                                { duration: 5000 }
                            )
                        })
                })
                .catch((err) => {
                    this.$toast.error('Något gick fel:' + err, {
                        duration: 5000,
                    })
                })
        },
        ...
        submitExport() {
            if (
                ...
                this.exportForm.addons.length == 0
            ) {
                this.$toast.warning(
                    'Du måste välja minst en lista att exportera'
                )
                return
            }
            if (this.to == 'textfile') {
            ...
            api.exportLists(this.to, this.exportForm).then((resp) => {
                if (this.to == 'protype') {
                    ...
                    link.setAttribute(
                        'download',
                        this.$store.state.userData.name.split(' ')[0] + '.zip'
                    )
                    ...
                    link.setAttribute(
                        'download',
                        this.$store.state.userData.name.split(' ')[0] + '.json'
                    )
                    document.body.appendChild(link)
                    link.click()
                    document.body.removeChild(link)
        ...
        changeSelectedJSONList() {
            ...
            this.countJSONAbbs =
                this.importableLists[this.selectedJSONList].length
        },
        uploadTxt() {
        ...
        resolveConflicts(abb) {
            ...
            if (this.listForm.addToExisting) {
                api.resolveConflicts(this.listForm.targetList.id, abbs)
                    .then((resp) => {
        ...
        onCancelImport() {
            this.$bvModal
                .msgBoxConfirm(
                    'Är du säker på att du vill avbryta importeringen?',
                    {
                        okTitle: 'Ja',
                        okVariant: 'danger',
        ...
        importAbbs() {
            ...
            if (this.listForm.addToExisting) {
                if (this.listForm.targetList != '') {
                    api.importList(this.listForm.targetList.id, this.abbs).then(
                        (resp) => {
                            EventBus.$emit(
                                'createdList',
                                this.listForm.targetList
                            )
                            this.reset()
                        }
                    )
                ...
                api.createList(list)
                    .then((resp) => {
                        api.importList(resp.data.id, this.abbs).then((resp) => {
