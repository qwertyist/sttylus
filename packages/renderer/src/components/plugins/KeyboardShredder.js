...
function traverseManuscript(action) {
    ...
    switch (action) {
        case 'promptWord':
            if (
                promptOnNewLine ||
                (manuscript.index == 0 && manuscript.line > 0)
            ) {
                newLine = true
            ...
            if (
                manuscript.index == manuscript.content[manuscript.line].length
            ) {
                manuscript.line += 1
                manuscript.index = 0
                newLine = true
        ...
        case 'removeWord':
            if (manuscript.index == 1 && manuscript.line == 0) {
                ...
                manuscript.line -= 1
                manuscript.index =
                    manuscript.content[manuscript.line].length - 1
                return
            }
            manuscript.index -= 1
            break
        ...
        case 'promptParagraph':
            if (manuscript.index > 0) {
                newLine = false
                text = manuscript.content[manuscript.line].slice(
                    manuscript.index
                )
                manuscript.index = 0
            } else {
        ...
        case 'promptClause':
            ...
            if (manuscript.content[manuscript.line].length > 1) {
                ...
                */
                if (
                    clauses.slice(1).length == clauses.slice(1).join('').length
                ) {
        ...
        state: {
            ...
            apply(tr, prev) {
                ...
                if (selection.from === selection.to) {
                    ...
                    const match = findWordBeforeCursor({ chars, anchor })(
                        $position
                    )
                    const decorationId = (Math.random() + 1)
                        .toString(36)
                        .substr(2, 5)
        ...
        props: {
            ...
            handleKeyDown(view, event) {
                if (
                    ...
                    view.dispatch(
                        view.state.tr.deleteRange(range.from, range.to + 1)
                    )

                    manuscript.abb = ''
                    return true
                }
                const { active, range, query, abbreviate } = this.getState(
                    view.state
                )
        ...
        if (prompt && manuscript.line < manuscript.content.length) {
                ...
                */
                if (
                    prompt &&
                    !event.ctrlKey &&
                    directions.indexOf(event.key) !== -1
                ) {
                    let chunks = []
              ...
              */
                            ...
                            if (
                                manuscript.content[manuscript.line] == undefined
                            ) {
                                prompt = false
                                console.log('stop prompting')
                                return
              ...
              */
                ...
                if (event.key == 'Enter') {
                    ...
                    let lastIndex = TextSelection.atEnd(view.state.doc).$anchor
                        .pos
