    ...
    return {
      ...
      "SHIFT-F2": state => {
        ...
        if (state.tr.selection.$from !== state.tr.selection.$to) {
          const selection = $position.doc.textBetween(state.tr.selection.from, state.tr.selection.to, '\0', '\0');

          this.options.addAbb(selection.trim().replace(/[\x00-\x1F\x7F-\x9F]/g, '').toLowerCase());
          return true;
        }
      ...
      F2: state => {
        ...
        if (state.tr.selection.$from !== state.tr.selection.$to) {
          ...
          this.options.addAbb(selection.trim().replace(/[\x00-\x1F\x7F-\x9F]/g, ''));
          return true;
        }
