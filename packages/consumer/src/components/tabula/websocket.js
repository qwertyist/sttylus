...
export default class wsConnection {
    ...
    reconnect(tries) {
        ...
        self.websocket.onerror = (e) => {
            console.log("error while reconnecting, retry...")
            tries++
            setTimeout(() =>  { this.reconnect(tries)}, 500+(tries*250))
        }
        self.websocket.onclose = () => {}
        self.websocket.onmessage = (e) => { console.log("recv message while reconnecting:", e)}
    ...
    joinsession() {
        let JoinMessage = JSON.stringify({ type: this.mt.JoinSession, msg:"user" })
